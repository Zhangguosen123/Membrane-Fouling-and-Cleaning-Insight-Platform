...
# Industrial backwash defaults. The flux ratio is user-adjustable in the sidebar.
...
INDUSTRIAL_CHECKPOINTS = {
    "early_end": 0.25,
    "middle_front_end": 0.45,  # 25% early + first half of 40% middle
    "middle_end": 0.65,        # 25% early + full 40% middle
    "late_end": 1.00,
}
...
def predict_total_filtration_time(pollutant_type, J0, V):
    """Predict total filtration time T(s) from the selected pollutant model."""
    J0 = float(J0)
    V = float(V)
    ...
    if not np.isfinite(T) or T <= 0:
        raise ValueError(f"The predicted total filtration time is invalid: {T}")
    ...
    return {
        "T": float(T),
        "early_end": float(INDUSTRIAL_CHECKPOINTS["early_end"] * T),
        "middle_front_end": float(INDUSTRIAL_CHECKPOINTS["middle_front_end"] * T),
        "middle_end": float(INDUSTRIAL_CHECKPOINTS["middle_end"] * T),
        "late_end": float(T),
...
def build_industrial_stage_analysis(t, J, predicted_schedule):
    """Build mechanism diagnosis windows used by the industrial backwash logic."""
    windows = [
        (
            "Early stage diagnosis (0-25%T)",
            "0-25%T",
            0.0,
            predicted_schedule["early_end"],
            "early",
        ),
        (
            "Middle stage first-half diagnosis (25-45%T)",
            "25-45%T",
            predicted_schedule["early_end"],
            predicted_schedule["middle_front_end"],
            "middle_front",
        ),
        (
            "Middle stage second-half diagnosis (45-65%T)",
            "45-65%T",
            predicted_schedule["middle_front_end"],
            predicted_schedule["middle_end"],
            "middle_back",
        ),
    ]
...
def find_flux_threshold_time_from_fit(params, J0, total_time, flux_ratio):
    t_grid = np.linspace(0.0, float(total_time), 800)
    J_grid = stage1_model(params, t_grid, J0)
    target_flux = J0 * flux_ratio
    crossing_time, crossing_idx = find_time_when_curve_below(t_grid, J_grid, target_flux)

    if crossing_time is None:
        return float(total_time), float(J_grid[-1]), len(t_grid) - 1, False

    return crossing_time, float(target_flux), crossing_idx, True


def find_empirical_cleaning_time(t, J_pred, J0, flux_ratio):
    target_flux = J0 * flux_ratio
...
def decide_backwash_timing(stage_results,
        ...
        if flux_threshold_reached and flux_threshold_time <= candidate_time:
            return {
                ...
                "basis": (
                    ...
                    "Once the membrane flux reaches the configured 50-70% J0 threshold, the cycle should not continue."
                ),
            }
    ...
    early_end = predicted_schedule["early_end"]
    mid_front_end = predicted_schedule["middle_front_end"]
    mid_end = predicted_schedule["middle_end"]
    ...
    if early is None:
        return cap_by_flux(
            ...
            "Stage-resolved mechanism analysis is incomplete. Backwashing is scheduled at the middle-stage end as a conservative safety point.",
            "The early diagnosis window is unavailable, so the platform falls back to the middle-end operating checkpoint under the flux-threshold cap.",
            "SAFE",
        )
    ...
    if early["intermediate_ratio"] < intermediate_threshold:
        return cap_by_flux(
            ...
            (
                f"Backwashing is recommended at the early-stage endpoint ({early_end:.2f} s). "
            ...
            (
                "Low intermediate blocking in the early 25% window suggests that the pore-entrance/interfacial "
                "bridging contribution has already weakened. Early hydraulic backwashing is preferred before "
                "the fouling structure consolidates."
            ),
            "E-IB",
        )
    ...
    if early["complete_to_intermediate_ratio"] > COMPLETE_TO_INTERMEDIATE_TRIGGER:
        return cap_by_flux(
            ...
            (
                "A complete-blocking contribution greater than intermediate blocking indicates stronger pore-blocking risk. "
                "The platform therefore triggers backwashing at the first diagnosis checkpoint."
            ),
            "E-CI",
        )

    if flux_threshold_reached and flux_threshold_time <= mid_front_end:
        return cap_by_flux(mid_front_end, "", "", "", "F")
    ...
    if mid_front is not None:
        if mid_front["complete_to_intermediate_ratio"] > COMPLETE_TO_INTERMEDIATE_TRIGGER:
            return cap_by_flux(
                ...
                (
                    f"Backwashing is recommended at the end of the first half of the middle stage "
                    f"({mid_front_end:.2f} s). The complete/intermediate blocking ratio is "
                    f"{mid_front['complete_to_intermediate_ratio']:.2f} > 1."
                ),
                (
                    "The complete/intermediate ratio criterion is activated before the late stage, so the platform "
                    "recommends intervention at the middle-front checkpoint."
                ),
                "MF-CI",
            )

        if early["intermediate_ratio"] > EPS:
            relative_drop = (early["intermediate_ratio"] - mid_front["intermediate_ratio"]) / early["intermediate_ratio"]
        else:
            relative_drop = 0.0
        ...
        if relative_drop > intermediate_drop_threshold:
            return cap_by_flux(
                ...
                (
                    ...
                    f"from early stage to the first half of the middle stage, exceeding the "
                    f"{intermediate_drop_threshold * 100:.2f}% criterion."
                ),
                (
                    "A rapid decline in intermediate blocking is treated as a warning of structural transition "
                    "from reversible interfacial fouling toward a more compact fouling layer."
                ),
                "MF-DROP",
            )

    if flux_threshold_reached and flux_threshold_time <= mid_end:
        return cap_by_flux(mid_end, "", "", "", "F")
    ...
    if mid_back is not None and mid_back["complete_to_intermediate_ratio"] > COMPLETE_TO_INTERMEDIATE_TRIGGER:
        return cap_by_flux(
            ...
            (
                f"Backwashing is recommended at the end of the middle stage ({mid_end:.2f} s). "
                f"The complete/intermediate blocking ratio in the second half of the middle stage is "
                f"{mid_back['complete_to_intermediate_ratio']:.2f} > 1."
            ),
            (
                "The complete/intermediate blocking ratio criterion is activated in the second half of the middle stage. "
                "Late-stage operation is not recommended."
            ),
            "MB-CI",
        )
    ...
    return cap_by_flux(
        ...
        (
            f"No early intermediate-blocking disappearance, rapid intermediate decline, or complete/intermediate "
            f"ratio trigger is detected. Backwashing is scheduled at the middle-stage endpoint ({mid_end:.2f} s)."
        ),
        (
            "The industrial rule avoids waiting until the late stage. If no mechanism-based early trigger appears, "
            "the second half of the middle stage endpoint is used as the planned cleaning intervention."
        ),
        "MID-END",
    )
...
def analyze_dataset(t, J, J0, filename, data_type, planned_volume,
    ...
    predicted_schedule = predict_total_filtration_time(data_type, J0, planned_volume)

    empirical_time, empirical_flux, empirical_idx, empirical_reached = find_empirical_cleaning_time(
        t, J_pred_full, J0, flux_ratio
    )

    flux_threshold_time, flux_threshold_flux, flux_threshold_idx, flux_threshold_reached = find_flux_threshold_time_from_fit(
        full["params"],
        J0,
        predicted_schedule["T"],
        flux_ratio,
    )

    t_empirical = t[: empirical_idx + 1]
    J_empirical = J[: empirical_idx + 1]
    empirical_stage = analyze_interval(
        t_empirical,
        J_empirical,
        f"Empirical 100%-{flux_ratio * 100:.0f}% Flux Operation Window",
    )

    industrial_stages = build_industrial_stage_analysis(t, J, predicted_schedule)
...
def draw_flux_curve(res):
    ...
    empirical_flux_line = res["J0"] * res["flux_ratio"]
    ax.axhline(
    ...
    schedule = res["predicted_schedule"]
    for label, x in [
        ("0.25T", schedule["early_end"]),
        ("0.45T", schedule["middle_front_end"]),
        ("0.65T", schedule["middle_end"]),
    ]:
        ax.axvline(x=x, color="#64748b", linestyle=":", alpha=0.5)
        ax.text(x, ax.get_ylim()[1] if ax.get_ylim()[1] > 0 else np.max(J), label, rotation=90, va="top", ha="right")
...
def main():
    ...
    intermediate_threshold_percent = st.sidebar.slider(
        ...
        help=(
            ...
            "backwashing is recommended at 0.25T unless the flux threshold is reached earlier."
        ),
    )
    ...
    intermediate_drop_percent = st.sidebar.slider(
        ...
        help=(
            ...
            "to the first half of middle stage, backwashing is recommended at 0.45T."
        ),
    )
    ...
    planned_volume = st.sidebar.number_input(
        ...
        value=300.0,
        step=1.0,
        help="Used with the pollutant-specific empirical formula to predict total filtration time T.",
    )

    st.sidebar.caption("Industrial checkpoints: 0.25T, 0.45T, and 0.65T. Late-stage backwashing is not considered.")
    ...
    if analysis_mode == "Single Preset File":
        ...
        st.info("The app will analyze BSAdata1.csv, HAdata1.csv, SAdata1.csv, and Mixturedata1.csv from the repository.")
    ...
    for res in all_results:
        ...
        summary_rows.append({
            ...
            "Planned Volume": f"{res['planned_volume']:.2f}",
            "Predicted T (s)": f"{res['predicted_schedule']['T']:.2f}",
    ...
    for res in all_results:
        ...
        with st.expander(f"Detailed Report: {res['filename']}", expanded=True):
            ...
            with col1:
                ...
                st.write(f"Predicted Total Time T: {res['predicted_schedule']['T']:.2f} s")
                st.write("Industrial Checkpoints: 0.25T / 0.45T / 0.65T")
                st.write(f"Forced Flux Threshold: {res['flux_ratio'] * 100:.1f}% of J0")
                if res["predicted_schedule"]["is_extrapolated"]:
                    st.warning("J0 or V is outside the fitted model range. Use this as a preliminary time window.")
            ...
            st.markdown("#### Total-Time Prediction Model")
            schedule = res["predicted_schedule"]
            model_df = pd.DataFrame([{
                "Pollutant": res["data_type"],
                "Formula": schedule["model"],
                "Predicted T (s)": round(schedule["T"], 2),
                "0.25T Early End (s)": round(schedule["early_end"], 2),
                "0.45T Middle First-Half End (s)": round(schedule["middle_front_end"], 2),
                "0.65T Middle End (s)": round(schedule["middle_end"], 2),
                "R2": schedule["r2"],
                "RMSE (s)": schedule["rmse"],
                "LOOCV RMSE (s)": schedule["loocv_rmse"],
            }])
